...
private:
    ...
    vector<pair<int, pair<int, int>>> edges;  // 边的集合 (权重, (u, v))
...
public:
    ...
    void dijkstra(int start) {
        ...
        vector<int> prev(n, -1);  // 存储路径中的前驱节点
        dist[start] = 0;

        vector<bool> visited(n, false);  // 标记节点是否已经访问
        priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> pq;  // 最小堆

        pq.push({ 0, start });  // 从起点开始
    ...
    void kruskalMST() {
        ...
        vector<pair<int, int>> mstEdges;  // 存储最小生成树的边
    ...
    int findParent(int u, vector<int>& parent) {
        ...
        return parent[u] = findParent(parent[u], parent);  // 路径压缩
    }
